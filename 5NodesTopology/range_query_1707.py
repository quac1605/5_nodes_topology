...
# ─────────────────────────────────────────────────────────────────────────────
# 3) Multi‑interval decomposition of the 5‑D query box
# ─────────────────────────────────────────────────────────────────────────────
...
def range_query(
    ...
    # half-open grid bounds: +1 on max_pt to include that coordinate
    min_pt = []
    max_pt = []
    for d in range(N):
    ...
    # decompose into precise Hilbert intervals
    hc        = HilbertCurve(P, N)
    intervals = decompose_hilbert_ranges(hc, min_pt, max_pt)

    # bisect each and union
    candidates = set()
    ...
    # final exact filter: RTT ≤, RAM ≥, cores ≥
    return sorted(
...
if __name__ == "__main__":
    ...
    for node in sorted(coords):
        ...
        h   = hilb_map[node]
        hd  = abs(h - h_curr)
        print(f"  {node:25s} RTT={rtt:6.2f}ms  RAM={ram:2d}GB  Cores={cores:2d}  "
              f"Hilbert={h}  HilbertDist={hd}")
